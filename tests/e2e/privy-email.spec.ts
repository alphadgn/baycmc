 ...
 *
 * The spec asserts two independent things:
...
test("Privy email input focuses on mobile, panel reports CSP block when iframe is denied", async ({
  page,
}) => {
  ...
  // Open the entrance modal.
  await page
    .getByRole("button", { name: /entrance|enter|verify|sign in/i })
    .first()
    .click();

  // Tap "Connect wallet" to open Privy. The button is rendered once the
  ...
  // Race: either Privy's email input mounts (allowlisted) or the
  // troubleshoot panel reports a CSP block.
  ...
  if (result === "email") {
    // Allowlisted path: focus the input and confirm focus stuck (this is
    // what the mobile keyboard hooks into).
    await emailInput.tap();
    await emailInput.focus();
    const focused = await emailInput.evaluate(
      (el) => el === document.activeElement,
    );
    expect(focused, "email input should be focused after tap").toBe(true);
  ...
  // Blocked path: the panel must explicitly call out the CSP / frame-ancestors
  // failure mode, and the allowlist textarea must be present and copyable.
  ...
  // We expect at least one CSP-shaped console message to have fired.
  expect(cspMessages.length, "expected a CSP/frame-ancestors console error")
    .toBeGreaterThan(0);
});
